#!/usr/bin/env python3
"""
Log documents to Argilla for human-in-the-loop review - Compatible with Argilla 2.0+
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Log documents to Argilla for human review (v2 for Argilla 2.0+)"
    )
    parser.add_argument(
...
def init_argilla(args):
    """Initialize Argilla client and dataset using Argilla 2.0+ SDK."""
    ...
    try:
        ...
        dataset_exists = any(ds.name == args.dataset_name for ds in datasets)
        
        if dataset_exists:
            logger.info(f"Dataset '{args.dataset_name}' already exists.")
    ...
    dataset = rg.FeedbackDataset.for_text_classification(
        ...
        metadata_properties=[
            ...
            rg.MetadataProperty(name="confidence", title="Model Confidence", type="float"),
            rg.MetadataProperty(name="reports_prob", title="Reports Probability", type="float"),
            rg.MetadataProperty(name="regulations_prob", title="Regulations Probability", type="float"),
            rg.MetadataProperty(name="file_path", title="File Path")
        ]
    )
    
    # Push dataset to Argilla
    dataset.push_to_argilla(name=args.dataset_name, workspace=args.workspace)
    logger.info(f"Created new dataset: {args.dataset_name}")
...
def create_argilla_records(dataset, model, tokenizer, args):
    """Create Argilla records from dataset using Argilla 2.0+ format."""
    records = []
    ...
    for i, example in enumerate(dataset):
        ...
        # Create Argilla record using new format
        record = rg.FeedbackRecord(
            ...
            suggestions=[
                rg.TextClassificationSuggestion(
